  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 bg-background/60 backdrop-blur-xl border-b border-primary/10">
      <div className="container mx-auto px-6 py-5">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2 group micro-interaction">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center shadow-gold hover:shadow-glow transition-all duration-500 group-hover:rotate-12">
              <Crown className="w-4 h-4 text-luxury-black" />
            </div>
            <h1 className="text-lg md:text-xl font-display font-black text-luxury-gradient group-hover:scale-105 transition-transform luxury-text-shadow">SavannahDynasty</h1>
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-6">
            ...
            if (item.hasSubmenu) {
              return <Popover key={item.path} open={aboutPopoverOpen} onOpenChange={setAboutPopoverOpen}>
                    <PopoverTrigger asChild>
                      <button className={`text-xs font-semibold transition-all duration-300 relative group micro-interaction flex items-center gap-1 ${isActive(item.path) ? "text-primary luxury-text-shadow" : "text-foreground/70 hover:text-primary"}`}>
                        {item.name}
            ...
            return <Link key={item.path} to={item.path} className={`text-xs font-semibold transition-all duration-300 relative group micro-interaction ${isActive(item.path) ? "text-primary luxury-text-shadow" : "text-foreground/70 hover:text-primary"}`}>
                  {item.name}
                  <span className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-primary to-accent transition-all duration-300 rounded-full ${isActive(item.path) ? "w-full shadow-glow" : "w-0 group-hover:w-full"}`} />
                </Link>;
          })}
            
            {isAdmin && <Link to="/admin/cats" className={`text-xs font-semibold transition-all duration-300 relative group micro-interaction ${isActive("/admin/cats") ? "text-primary luxury-text-shadow" : "text-foreground/70 hover:text-primary"}`}>
                ⚙️ Админ
